 ...
 * Change to prevent event handler installation:  Rhythm Engineering, LLC 2014
...
;(function ($, window, document, undefined) {
    ...
    $.fn.mapbox = function (options, callback) 
	{
		...
		window.markerCoords = {lat: -200, lon: -200};
		...
		function pointsToCoords(x, y)
		{
			...
			var height = width * 0.506;

			y -= 1057 * (width / 4278);

			var lonCenter = width / 2;
			var latCenter = height / 2;
		...
		function coordsToPoints(lon, lat)
		{
			var layer = $("#viewport").find(".current-map-layer");
			...
			x = centerX + x + layer.position().left;
			y = centerY + y + layer.position().top;
        ...
        else if(options == "markerCoords")
		{
			return markerCoords;
		} 
		else if(options == "setCoords")
		{
			markerCoords.lat = callback.lat;
			markerCoords.lon = callback.lon;
			var points = coordsToPoints(markerCoords.lon, markerCoords.lat);
			$(".marker").show();
			$(".marker").css({left:points.x,top:points.y});
			markerShow = true;
			return;
		}
        ...
        return this.each(function() {
            ...
            // Else setup the map.
				...
				if(options.installHandlers)
				{
		            ...
		            $(document).mouseup(function(e) 
					{
						...
						if(!weveMoved)
						{
							...
							if(mouseX >= $(viewport).offset().left 
									&& mouseX <= $(viewport).offset().left + $(viewport).width()
									&& mouseY >= $(viewport).offset().top
									&& mouseY <= $(viewport).offset().top + $(viewport).height())
							{						
								var layerX = layer.position().left;
								var layerY = layer.position().top;

								var viewportX = $("#viewport").offset().left;
								var viewportY = $("#viewport").offset().top;

								var mouseViewportPosX = mouseX - viewportX;
								var mouseViewportPosY = mouseY - viewportY;

								var markerX = mouseViewportPosX - 16;
								var markerY = mouseViewportPosY - 32;

								$(".marker").show();
								$(".marker").css({left:markerX,top:markerY});

								var pointX = mouseViewportPosX - layerX;
								var pointY = mouseViewportPosY - layerY;

								var coords = pointsToCoords(pointX, pointY);
                ...
                // Deferred, load img in hidden layers
